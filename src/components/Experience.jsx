  ...
  return (
    <section id="experiencia" className="py-20 flex flex-col md:flex-row gap-12">
      ...
      <div className="md:w-1/3 text-white">
        ...
        <p className="mt-4 text-slate-500 text-sm">Proyectos desarrollados durante mi formación profesional.</p>
      </div>
      
      {/* Lista de Experiencias */}
      <div className="md:w-2/3 space-y-8">
        {experienceData.map((exp, index) => {
          const ProjectIcon = getIcon(exp.icon);
          
          return (
            <div key={index} className="bg-[#0f172a] border border-slate-800 p-8 rounded-[2rem] space-y-6 hover:border-blue-500/30 transition-all duration-300">
              <div className="flex justify-between items-start gap-4">
                <div className="space-y-2">
                  <span className="text-emerald-400 text-[10px] font-black px-3 py-1 bg-emerald-500/10 rounded-full flex items-center gap-2 w-fit uppercase tracking-wider">
                    <CalendarIcon size={12} /> Proyecto de Estancia
                  </span>
                  <h3 className="text-2xl font-bold text-white leading-tight">{exp.title}</h3>
                  <p className="text-blue-500 font-bold text-xs uppercase tracking-widest">{exp.sector}</p>
                </div>
                <div className="p-4 bg-slate-800/50 text-blue-500 rounded-2xl border border-slate-700">
                  <ProjectIcon size={28} />
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {exp.tech.map((t, i) => (
                  <span key={i} className="text-[10px] font-bold px-2 py-1 bg-slate-900 text-slate-300 rounded-md border border-slate-800">
                    {t}
                  </span>
                ))}
              </div>
              ...
              {/* Puntos clave */}
              <ul className="space-y-3 text-slate-400 text-sm border-t border-slate-800/50 pt-4">
